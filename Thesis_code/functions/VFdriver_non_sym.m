function [SER,rmserr,bigHfit,opts]=VFdriver_non_sym(bigH,s,poles,opts); 
...
if VF.spy2==1
  ...
  if VF.logx==1     
    ...
    if VF.phaseplot==1    
      ...
      % Unwrap the phase data to remove discontinuities
      phase_orig = unwrap(angle(f));
      phase_vf = (angle(fit));
        
      % Convert back to degrees
      phase_orig = rad2deg(phase_original_unwrapped);
      phase_vf = rad2deg(phase_vector_fit_unwrapped);

      h4=plot(freq,phase_orig,'b'); xlim([freq(1) freq(Ns)]);hold on
      h5=plot(freq,phase_vf,'r--');hold off
    end  
  end %logy=0
  figure(1000),
